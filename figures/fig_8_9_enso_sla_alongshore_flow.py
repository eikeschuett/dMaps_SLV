...
extent1 = [-75, -70, -30, -20]
extent2 = [-80, -70, -20, -10]
extents = [extent1, extent2]#, extent1]
regions = ["chile","peru"]
...
for i, extent in enumerate(extents):
    ...
    #%%
    fig = plt.figure(figsize=(12,8))#18,12))
    ...
    nau.calc_ax_cross_corr(pd.Series(signals[:,5]), v_north, time, ax=ax2,
    ...
    nau.calc_ax_cross_corr(pd.Series(signals[:,5]), v_east, time, ax=ax3,
                       lag_range=range(12,-13,-1),
                       out_fname = None,
                       return_crosscorr = False,
                       #data1_lead_label="SLA",
    ...
    out_fname = "dMaps_SLV/figures/fig_{nr}_SLA_5_flow_{region}.png".format(
        nr=i+8,
        region=regions[i])
